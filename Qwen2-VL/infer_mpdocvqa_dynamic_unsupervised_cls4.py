...
import torch
from model import Qwen2VLForConditionalGeneration_Unsupervised_CLS3
...
def collate_fn(batches):
    ...
    return inputs, questions, question_ids, annotations, num_token_list, messages_high, messages_low, answer_page_idxs, num_image_list
...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_paths, query, question_id, annotation, answer_page_idx = data['images'], data[
            'query'].replace('<image>',''), data['id'], data.get('answers', None), data.get('answer_page_idx', -1)
...
def predict_page(
    ...
    attentions = tmp_layer_outputs[1]
    ...
    if attn_pool == 'mean':
        ...
        attn_weights_for_page_pred = attn_weights.clone()
        attn_weights_for_page_pred = nn.functional.softmax(attn_weights_for_page_pred, dim=-1)

        threshold = attn_weights.mean()
        # attn_weights[attn_weights <= threshold] -= 1e7
        compresspage_tmp = torch.where(attn_weights <= threshold)[0]

        attentions = attn_weights_for_page_pred
        # print(attn_weights_for_page_pred)
   ######################################################################

    if select_mode == 'dynamic':
        page_pred = torch.where(attentions != 0)[0]
    elif select_mode == 'mix_dynamic':
        page_pred = torch.where(attentions != 0)[0]
        # if len(pixel_values) > 15:
        page_drop = torch.where(attentions == 0)[0]
        # else:
            # page_drop = []
        return page_pred, page_drop
    elif select_mode == 'mix_topk':
        page_pred = torch.topk(attentions, min(select_image_num, len(num_token_list[0])), dim=-1).indices

        if len(num_token_list[0]) > 8:
            page_drop = torch.tensor([pid for pid in compresspage_tmp if pid not in page_pred])
        else:
            page_drop = []
        # print(select_image_num, len(num_token_list[0]), page_pred, len(page_drop))
        return page_pred, page_drop
    else:
...
def get_new_input(page_pred,
    ...
    for idx in range(len(images_low)):
        if idx in page_pred:
            ...
            if idx in page_drop:
                images_low[idx]['max_pixels'] = images_low[idx]['max_pixels'] // 2
    ...
    avg_vtokens = sum(new_inputs['input_ids'][0]==151655) // len(images_low)
    total_tokens = len(new_inputs['input_ids'][0])

    return new_inputs, avg_vtokens, total_tokens

@torch.no_grad()
def evaluate_chat_model(model, processor, image_process_args, select_mode, select_image_num):
    ...
    for ds_name in args.datasets:
        ...
        for _, (inputs, questions, question_ids, annotations, num_token_list, messages_high, messages_low, answer_page_idxs, num_image_list) in tqdm(enumerate(dataloader)):
            # get page pred
            try:
                if len(num_token_list[0]) > select_image_num:
                    page_pred, page_drop = predict_page(inputs[0].to(model.device),
                                ...
                                select_image_num=select_image_num)
                    ...
                    inputs, avg_vtokens, total_tokens = get_new_input(page_pred=page_pred,
                                            page_drop=page_drop,
                    ...
                    # avg_vtokens = sum(inputs[0]['input_ids'][0]==151655) // len(num_token_list[0])
                    # total_tokens = len(inputs[0]['input_ids'][0])
                    page_pred = list(range(len(num_token_list[0])))
                    page_drop = []
                    inputs, avg_vtokens, total_tokens = get_new_input(page_pred=page_pred,
                                            page_drop=page_drop,
                                            messages_high=messages_high[0][0],
                ...
                answers = pred
                avg_vtokens_list.append(avg_vtokens.cpu())
                total_tokens_list.append(total_tokens)
                print(pred, avg_vtokens.cpu().item(), total_tokens)
        ...
        torch.distributed.all_gather_object(merged_avg_vtokens, avg_vtokens_list)

        merged_total_tokens = [None for _ in range(world_size)]
        torch.distributed.all_gather_object(merged_total_tokens, total_tokens_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-pixels-low', type=float, default=200704)
    parser.add_argument('--max-pixels-high', type=float, default=802816)
    parser.add_argument('--dynamic', action='store_true')
    parser.add_argument('--out-dir', type=str, default='results')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--select-mode', type=str, default='mix_topk')
    parser.add_argument('--select-image-num', type=int, default=3)
    ...
    model = Qwen2VLForConditionalGeneration_Unsupervised_CLS3.from_pretrained(
            args.checkpoint,
    ...
    image_process_args = {
        "max_pixels_low": args.max_pixels_low,
        "max_pixels_high": args.max_pixels_high,
    ...
    evaluate_chat_model(model, processor, image_process_args, args.select_mode, args.select_image_num)
