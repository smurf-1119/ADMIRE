...
def predict_page(
    ...
    image_embeds = qwen2vl_generation_model.visual(pixel_values, grid_thw=image_grid_thw)
...
def evaluate_chat_model(model, processor, image_process_args, select_mode, select_image_num):
    ...
    for ds_name in args.datasets:
        ...
        for _, (inputs, questions, question_ids, annotations, num_token_list, messages_high, messages_low, answer_page_idxs, num_image_list) in tqdm(enumerate(dataloader)):
            # get page pred
            page_pred, page_drop = predict_page(inputs[0].to('cuda'),
        ...
        if torch.distributed.get_rank() == 0:
            ...
            pred_acc = torch.tensor(pred_acc).float().mean(-1)
            print(pred_acc)
            ...
            for key_index in range(5):
                ...
                json.dump(outputs_details[key_index], open(results_file_index, 'w'))
