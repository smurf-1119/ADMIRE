...
import torch
from model import Qwen2VLForConditionalGeneration_Unsupervised_CLS
...
def collate_fn(batches):
    ...
    return inputs, questions, question_ids, annotations, num_token_list, messages_high, messages_low
...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for idx in range(1,len(vision_pos_pad)):
            if vision_pos_pad[idx] - vision_pos_pad[idx] > 1:
...
def evaluate_chat_model(model, processor, image_process_args):
    ...
    for ds_name in args.datasets:
        ...
        for _, (inputs, questions, question_ids, annotations, num_token_list, messages_high, messages_low) in tqdm(enumerate(dataloader)):
            generated_ids = model.generate(**inputs[0].to(model.device),
                                           max_new_tokens=1024,
                                           num_token_list=num_token_list,
                                           attn_pool='max',
                                           cls_num_layers=1,
            ...
            for question, question_id, answer, annotation in zip(questions, question_ids, answers, annotations):
                if ds_name in ['dude_mpdocvqa_val', 'mpdocvqa_val', 'mpdocvqa_train1000', 'docvqa_val', 
                                 'mpdocvqa_2stage_val', 'dude_mpdocvqa_2stage_val', 'newsvqa_val']:
                    outputs.append({
                        'question': question,
                    ...
                    key_index = len(num_patches_list[0]) // 5
                    if key_index > 4:
                        key_index = 4
...
if __name__ == '__main__':
    ...
    model = Qwen2VLForConditionalGeneration_Unsupervised_CLS.from_pretrained(
            args.checkpoint,
    ...
    image_process_args = {
        "max_pixels_low": args.max_pixels * 5,
